 ...
 */
void add_edge(Graph *g, int u, int dest, long long w /*, short int is_mulinello*/)
{
    ...
    node->dest = dest;
    node->weight = w;
    // node->is_mulinello = is_mulinello;
    node->next = g->adj[u];
    g->adj[u] = node;
 ...
 */
void dijkstra(Graph *graph, int src, long long dist[], short int *da_mulinello)
{
...
int main()
{
    ...
    short int *da_mulinello = malloc(n * sizeof(short int));
    if (dist == NULL || da_mulinello == NULL)
    {
    ...
    for (i = 0; i < k; i++)
    {
        ...
        // Comincio a popolare le distanze e se arrivo da un mulinello o meno
        if (dist[s] > y)
            dist[s] = y;
    ...
    dijkstra(&g, 0, dist, da_mulinello);

    // Conto il numero di volte che ho usato un mulinello
    int count = 0;
    for (i = 0; i < g.num_nodes; i++)
    {
        if (da_mulinello[i] == 1)
